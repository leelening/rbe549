# -*- coding: utf-8 -*-
"""Copy of NeRF From Nothing

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/14z_BBr20JbOdTkzeBQUbJETHIDsGGl0S
...
images = np.stack(data["original_images"][0], axis=0)
poses = np.stack(data["poses"][0], axis=0)
focal = np.array([917.665738377035, 825.174331721588])
...
def get_rays(
    ...
    directions = torch.stack(
        [
            (i - width * 0.5) / focal_length[0],
            -(j - height * 0.5) / focal_length[1],
            -torch.ones_like(i),
        ],
        dim=-1,
    )
...
class PositionalEncoder(nn.Module):
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        """
        return torch.concat([fn(x.float()) for fn in self.embed_fns], dim=-1)
...
for i in range(n_restarts):
    print("{}\% training finished".format(i / n_restarts))
    (
        model,
        fine_model,
        encode,
